...
for (let i = 0; i < 40; i++) {
  const act = await ev(
    `(async () => { const a = await window.api.getWorktreeActivity(); return (a||[]).filter(x => ${JSON.stringify(CIBLES)}.includes(x.agentId)).map(x => ({ id: x.agentId, s: x.state, p: x.publication, r: x.attentionReason, wt: x.worktreeAvailable })) })()`
  )
  const encore = act.filter((x) => x.s === 'working' || x.s === 'isolated')
  console.log(new Date().toISOString().slice(11, 19), JSON.stringify(act))
  if (!encore.length && act.length) {
