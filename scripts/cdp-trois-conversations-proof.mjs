...
const TACHE =
  "Ajoute une seule ligne a la fin du fichier PREUVE-TROIS-CONVERS.md a la racine du depot : une ligne commencant par '- ' suivie d'un mot au hasard. Ne touche a AUCUN autre fichier."

const ids = []
for (const nom of ['preuve-1', 'preuve-2', 'preuve-3']) {
  ids.push(
    await ev(
      `(async () => { const c = await window.api.conversationsCreate(${JSON.stringify(nom)}); return c?.id ?? c })()`
    )
  )
}
