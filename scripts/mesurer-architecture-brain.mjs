...
function categorieDe(rel, tags) {
  ...
  if (a('environnement', 'environment')) return 'Environnement'
  if (seg[0] === 'integrations') return 'Environnement'
...
const A =
  ...
  NL +
  [...parDossier.entries()].sort().map(([d, n]) => `  ${d}/  (${n})`).join(NL)

// --- SURFACE B : les catégories cognitives, puis leurs sous-groupes.
const ORDRE = ['Comportement', 'Mémoires', 'Environnement', 'Savoir', 'Documentation', 'Code', 'À trier', 'Non classé']
...
for (const c of ORDRE) {
  ...
  B += NL + `${c} (${tot})` + NL + [...m.entries()].sort().map(([s, n]) => `    ${s}  (${n})`).join(NL) + NL
}
...
const C =
  ...
  NL +
  toutes.slice().sort((a, b) => a.rel.localeCompare(b.rel)).map((f) => `  ${f.rel} — ${f.titre}`).join(NL)

for (const [nom, txt] of [['A-dossiers', A], ['B-categories', B], ['C-plate', C]]) {
  writeFileSync(join(SORTIE, nom + '.txt'), txt, 'utf8')
  console.log(`  ${nom.padEnd(14)} ${String(txt.length).padStart(7)} caractères  ${String(txt.split(NL).length).padStart(5)} lignes`)
}
...
for (const c of ORDRE) {
  ...
  console.log(`  ${c.padEnd(14)} ${String(n).padStart(4)}   ${((100 * n) / toutes.length).toFixed(1)} %`)
}
