...
/**
 * SCINDER — l'inverse du fork. Mesure du 2026-09-13 (cost.jsonl) : 1 952 tours de chat dépassent
 * 400 000 jetons d'entrée et coûtent 2 563 $, soit 62 % de la dépense. Forker DOUBLE ce passé ;
 * scinder le DÉPLACE, et la source retrouve son poids d'avant la rupture de sujet.
 */
describe('ConversationStore — split', () => {
  it('déplace la suite dans un fil neuf ET l’enlève de la source', () => {
    const { store, id, ids } = seed()
    const { source, cible } = store.split(id, ids[2]) // à partir de u2

    expect(cible.id).not.toBe(id)
    expect(cible.messages.map((m) => m.content)).toEqual(['u2', 'a2'])
    expect(source.messages.map((m) => m.content)).toEqual(['u1', 'a1'])
    expect(store.get(id)!.messages.map((m) => m.content)).toEqual(['u1', 'a1'])
  })

  it('ne perd aucun message : source + cible = le fil d’origine', () => {
    const { store, id, ids } = seed()
    const { source, cible } = store.split(id, ids[1])
    expect([...source.messages, ...cible.messages].map((m) => m.content)).toEqual([
      'u1',
      'a1',
      'u2',
      'a2'
  ...
  it('les messages déplacés reçoivent de NOUVEAUX identifiants et des parents remappés', () => {
    const { store, id, ids } = seed()
    const { cible } = store.split(id, ids[1])

    for (const message of cible.messages) expect(ids).not.toContain(message.messageId)
    expect(cible.messages[0].parentMessageId).toBeUndefined()
    for (let index = 1; index < cible.messages.length; index += 1) {
      expect(cible.messages[index].parentMessageId).toBe(cible.messages[index - 1].messageId)
    }
  })

  it('garde la provenance et apparaît dans la liste', () => {
    const { store, id, ids } = seed()
    const avant = store.list().length
    const { cible } = store.split(id, ids[2])
    expect(cible.forkedFrom).toEqual({ conversationId: id, messageId: ids[2] })
    expect(store.list().length).toBe(avant + 1)
  })

  it('refuse de vider la source (scission au premier message) et l’id inconnu', () => {
    const { store, id, ids } = seed()
    expect(() => store.split(id, ids[0])).toThrow(/resterait vide/)
    expect(() => store.split(id, 'message-inconnu')).toThrow(/Message inconnu/)
    expect(() => store.split('conv-404', ids[1])).toThrow(/Conversation inconnue/)
    // Un refus ne touche à rien.
    expect(store.get(id)!.messages).toHaveLength(4)
  })
})
