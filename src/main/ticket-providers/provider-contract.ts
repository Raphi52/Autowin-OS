...
function assertSafeUrl(input: string): void {
  try {
    ...
    throw new TicketProviderError('UNSAFE_URL', 'Adresse fournisseur non s?re.')
  }
}
...
function errorForStatus(status: number): TicketProviderError {
  ...
  if (status === 403) return new TicketProviderError('ACCESS_DENIED', 'Acc?s refus?.')
  if (status === 429) return new TicketProviderError('RATE_LIMITED', 'Limite fournisseur atteinte.')
  return new TicketProviderError(
    'REMOTE_ERROR',
    `Le fournisseur a r?pondu avec le statut ${status}.`
  )
}
...
export async function fetchTicketJson<T>(
  ...
  try {
    ...
    if (options.signal?.aborted) {
      throw new TicketProviderError('ABORTED', 'Chargement annul?.')
    }
    if (timeoutSignal.aborted) {
      throw new TicketProviderError('TIMEOUT', 'D?lai fournisseur d?pass?.')
    }
  ...
  if (Number.isFinite(announcedLength) && announcedLength > maxBytes) {
    throw new TicketProviderError('RESPONSE_TOO_LARGE', 'R?ponse fournisseur trop volumineuse.')
  }
  ...
  try {
    ...
    throw new TicketProviderError('INVALID_RESPONSE', 'R?ponse fournisseur invalide.')
  }
}
    ...
    list: (request, context) => {
      ...
      if (!adapter) {
        throw new TicketProviderError(
          'UNSUPPORTED_PROVIDER',
          `Fournisseur non support? : ${request.source.provider}.`
        )
      }
