...
describe('adaptateur Azure DevOps Tickets', () => {
  it('parcourt tous les types et ?tats avec une requ?te WIQL ordonn?e et un curseur', async () => {
    const fetchFn = vi
      .fn()
  ...
  it('charge les d?tails par lots de 200 maximum et normalise champs et relations', async () => {
    const references = Array.from({ length: 202 }, (_, index) => ({ id: index + 1 }))
    const item = (id: number) => ({
      id,
      fields: {
        'System.WorkItemType': 'Type personnalis?',
        'System.Title': `?l?ment ${id}`,
        'System.State': '?tat personnalis?',
        'System.ChangedDate': '2026-07-23T10:00:00.000Z',
        'System.CreatedDate': '2026-07-22T10:00:00.000Z',
        'System.AssignedTo': { displayName: 'Ada Lovelace' },
        'Microsoft.VSTS.Common.Priority': 2,
        'System.Description': '<p>D?tail</p>',
        'Custom.Inconnu': 'conserv?'
      },
      relations: [
        {
    ...
    expect(page.items[0]).toMatchObject({
      ...
      type: 'Type personnalis?',
      title: '?l?ment 1',
      state: '?tat personnalis?',
      url: 'https://dev.azure.com/AmitelGTC/RIG/_workitems/edit/1',
      updatedAt: '2026-07-23T10:00:00.000Z',
      createdAt: '2026-07-22T10:00:00.000Z',
      assignee: 'Ada Lovelace',
      priority: 2,
      description: '<p>D?tail</p>',
      fields: expect.objectContaining({ 'Custom.Inconnu': 'conserv?' }),
      relations: [
        {
  ...
  it('classe une forme de r?ponse Azure invalide via le contrat fournisseur', async () => {
    ...
    ).rejects.toEqual(new TicketProviderError('INVALID_RESPONSE', 'R?ponse Azure DevOps invalide.'))
  })
})
