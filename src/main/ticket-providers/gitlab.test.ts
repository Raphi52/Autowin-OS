...
describe('adaptateur GitLab Issues', () => {
  ...
  it('utilise la base configur?e, la borne basse et termine sans page suivante', async () => {
    const fetchFn = vi.fn(async (_input: string | URL | Request, _init?: RequestInit) =>
      Response.json([])
    )
  ...
  it('rejette une charge utile qui ne contient pas une liste', async () => {
    ...
    ).rejects.toEqual(new TicketProviderError('INVALID_RESPONSE', 'R?ponse GitLab invalide.'))
  })
})
