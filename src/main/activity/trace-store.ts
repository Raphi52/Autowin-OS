...
export class TraceStore {
  ...
  append(event: TraceEventV1): this {
    ...
    const existing = this.ids.has(event.conversationId) ? undefined : this.readConversation(event.conversationId)
    ...
    const lastSequence = this.lastSequences.get(event.conversationId)
      ?? (existing?.length ? existing[existing.length - 1].sequence : -1)
    if (event.sequence <= lastSequence) throw new Error(`sequence non monotone: ${event.sequence} <= ${lastSequence}`)
    if (event.parentId && !seen.has(event.parentId)) throw new Error(`parent causal introuvable: ${event.parentId}`)
    mkdirSync(this.root, { recursive: true })
    const descriptor = this.descriptors.get(event.conversationId)
      ?? openSync(this.path(event.conversationId), 'a')
  ...
  exportConversation(conversationId: string): TraceEventV1[] { return this.readConversation(conversationId) }
  importConversation(events: TraceEventV1[]): this { for (const event of events) this.append(event); return this }
