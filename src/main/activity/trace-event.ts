...
const PARTICIPANT_KINDS = new Set<TraceParticipantKind>([
  'human', 'system', 'agent', 'judge', 'tool', 'provider', 'skill', 'hook', 'resource'
])
const PAYLOAD_KINDS = new Set<TracePayloadKind>([
  'user-message', 'system-instruction', 'app-state', 'history', 'resource', 'attachment',
  'tool-call', 'tool-result', 'model-response', 'error'
])
...
export function assertTraceEvent(event: TraceEventV1): TraceEventV1 {
  ...
  if (!Number.isFinite(Date.parse(event.timestamp))) throw new Error('TraceEvent: timestamp invalide')
  if (!Number.isSafeInteger(event.sequence) || event.sequence < 0) throw new Error('TraceEvent: séquence invalide')
  ...
  if (!Array.isArray(event.payloads) || event.payloads.length === 0) throw new Error('TraceEvent: payloads vides')
  ...
  if (!['exact', 'derived', 'opaque'].includes(event.observation.fidelity)) throw new Error('TraceEvent: fidélité invalide')
  for (const value of Object.values(event.metrics ?? {})) {
    if (value !== undefined && (!Number.isFinite(value) || value < 0)) throw new Error('TraceEvent: métrique invalide')
  }
  return event
}
