...
export function pilotActionToTraceEvent(input: PilotActionTraceInput): TraceEventV1 {
  const type: TraceEventV1['type'] =
    input.kind === 'command' ? 'tool-call' : input.kind === 'result' ? 'tool-result' : input.kind
    ...
    payloads: [
      {
        kind:
          input.kind === 'command'
            ? 'tool-call'
            : failed || input.kind === 'retry' || input.kind === 'cancellation'
              ? 'error'
              : 'tool-result',
        name: input.name,
        content:
          typeof input.data === 'string' ? input.data : JSON.stringify(input.data ?? null, null, 2)
      }
    ],
