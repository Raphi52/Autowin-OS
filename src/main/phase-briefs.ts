 ...
 *
 * ANCRAGES du brief SCOUT (hors chaînes injectées) :
 *  - Score chiffré obligatoire : mesuré le 2026-08-18 — 16 lignes non chiffrées d'affilée,
 *    shortlist inexploitable.
...
ANTI-BLOCAGE — un blocage inventé coûte un tour (conv-1286 : 21 tours pour une demande d'un tour).
- Demande ELLIPTIQUE ("vazy", "continue", "répare") = la RECOMMANDATION du tour précédent, telle quelle ; elle ne redéfinit pas la tâche ("réessaye en boucle" = réessayer LA tâche, pas réécrire le moteur de retry).
...
- SORTIE LONGUE : lance UNE fois, redirige vers un fichier, LIS sa fin ; ne RELANCE jamais une sortie tronquée, le fichier l'a déjà.
PARI — avant ta derniere ligne, ajoute exactement \`AUTOWIN_PARI_V1: {"confiance":<0..1>,"refutateur":"<ce qui te donnerait tort>"}\`. \`confiance\` = probabilite que le JUGE valide ce travail du premier coup : une prevision tranchee par un tiers, PAS ton ressenti. Non revisable ; 0,5 partout = absence d'information.
Leçon facultative : seulement si une leçon NOUVELLE, réutilisable et prouvée par le run existe, ajoute UNE ligne finale exacte \`AUTOWIN_LESSON_V1: {"outcome":"success|failure","title":"...","body":"...","type":"lesson|decision|preference|domain","scope":"project","tags":["..."],"confidence":"low|medium|high"}\` — JSON sur une seule ligne, aucun autre champ, \`scope\` = \`project\` ou \`global\`. Pour un échec, body distingue \`Tentative:\`, \`Symptôme:\`, \`Cause (prouvée):\` ou \`Cause (hypothèse):\`, \`Prochaine stratégie:\`. Aucune leçon plutôt qu'une généralisation sans preuve.`,
