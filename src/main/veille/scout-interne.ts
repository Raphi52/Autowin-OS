...
export function construirePromptScoutInterne(params: ParametresScoutInterne): string {
  return [
    ...
    '   (#d4a94f-#e3ba55) ; texte #dde3ee ; chemins en chips monospace discrètes (fond',
    '   rgba(255,255,255,.045), bordure rgba(255,255,255,.13)) ; jamais de halos, aucun JavaScript.',
    '   Contenu : les fichiers lus et 2-3 lignes de constats, C’EST TOUT — ne liste PAS les',
    '   candidats, l’application les affiche elle-même juste en dessous avec cases et détails.',
    '   COMPACT : interlignes 1.45-1.55, marges de section ≤10px, pas de grands vides — la synthèse',
    '   tient en quelques lignes denses, pas en page aérée.',
    ...
    '- ANCRE LA CAUSE, PAS LE SYMPTÔME. Pour un `correction`, `url` doit pointer le code qui',
    '  PRODUIT le défaut, jamais la ligne qui l’AFFICHE ni le message qui le décrit. Un message',
    '  d’erreur, un libellé, un `console.warn`, un texte de refus ne sont que l’endroit où le défaut',
    '  se VOIT. Mesuré le 2026-09-12 : un candidat a ancré « le refus annoncé définitif se réécrit 92',
    '  fois » sur `relaunch-resumable-run.ts:416`, la ligne du message ; la cause réelle était dans',
    '  `orchestration-state.ts`, où une écriture de checkpoint encore en vol renommait son fichier',
    '  APRÈS l’effacement et ressuscitait le run. Corriger à la ligne citée n’aurait rien réparé.',
    '- COMMENT TROUVER LA CAUSE avant d’écrire le candidat : pars du texte observé, trouve qui',
    '  l’ÉMET, puis remonte d’un cran à ce qui déclenche cette émission — l’état écrit ou effacé, la',
    '  condition qui redevient vraie, la boucle qui rappelle. Un défaut qui se RÉPÈTE se juge sur ce',
    '  qui RECRÉE sa condition, jamais sur la ligne répétée. Si tu n’as pas su remonter, dis-le dans',
    '  `why` (« cause non localisée ») plutôt que d’ancrer le symptôme : un ancrage faux coûte plus',
    '  cher qu’un candidat honnêtement incomplet.',
    ...
    '- `how` : COMMENT on le fait, concrètement (fichier à toucher, geste à faire) — au point de',
    '  CAUSE ancré, pas un garde ajouté autour du symptôme.',
