...
export interface CapabilityProfile {
  ...
  name: string
  description: string
  selections: Record<CapabilityKind, Record<string, boolean>>
  updatedAt: string
}
  ...
  profiles: [
    {
      id: 'lean',
      name: 'Lean',
      description: 'Contexte minimal, coût et surface d’action réduits.',
      selections: { skills: {}, hooks: {}, tools: {} },
      updatedAt: now()
    },
    {
      id: 'balanced',
      name: 'Équilibré',
      description: 'Profil standard : toutes les capacités découvertes sont autorisées.',
      selections: { skills: {}, hooks: {}, tools: {} },
      updatedAt: now()
    },
    {
      id: 'full',
      name: 'Complet',
      description: 'Profil d’exploration avec toutes les capacités autorisées.',
      selections: { skills: {}, hooks: {}, tools: {} },
      updatedAt: now()
    }
  ],
...
export function loadCapabilityProfiles(): CapabilityProfileState {
  try {
    ...
    const raw = JSON.parse(readFileSync(p, 'utf8')) as CapabilityProfileState
    return Array.isArray(raw.profiles) && raw.assignments ? raw : defaults()
  } catch {
    return defaults()
  }
}
