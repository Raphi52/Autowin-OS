...
export function parseGeneratedLoop(raw: string, allowedSkills: Set<string>): LoopRunInput {
  ...
  const steps = parsed.steps
    .map((step, index) => {
      ...
      if (
        /^(cadre le besoin|prepare le terrain|execute le travail|audite le resultat)/i.test(
          step.prompt.trim()
        )
      )
      ...
      return {
        id:
          typeof step.id === 'string' && /^[a-zA-Z0-9_-]{1,80}$/.test(step.id)
            ? step.id
            : `step-${index + 1}`,
        skill: step.skill,
        ...
        requires: Array.isArray(step.requires)
          ? step.requires.filter((x): x is string => typeof x === 'string')
          : [],
        produces: Array.isArray(step.produces)
          ? step.produces.filter((x): x is string => typeof x === 'string')
          : []
      }
    })
