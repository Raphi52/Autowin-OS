...
describe('capability-controls (source locale générique, sans Native)', () => {
  ...
  beforeEach(() => {
    ...
    // Le fixture passe par le même writer versionné que l'application : écrire à la main l'ancien
    // format partiel transformerait ce test de diff en test involontaire de migration legacy.
    seedRegistrySnapshot(
      {
        tools: [
          { id: 'alpha', label: 'alpha', description: '', enabled: true, mutable: true },
          { id: 'beta', label: 'beta', description: '', enabled: false, mutable: true },
          { id: 'gamma', label: 'gamma', description: '', enabled: false, mutable: true }
        ]
      },
      base
    )
  ...
  it('le registre natif applique une sélection cible par diff', async () => {
    ...
    for (const item of listNativeRegistry('tools', base)) {
      if (item.enabled !== target.has(item.id))
        setNativeEnablement('tools', item.id, target.has(item.id), base)
    }
