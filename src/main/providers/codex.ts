...
export class CodexAdapter implements ProviderAdapter {
  ...
  private async accessToken(): Promise<string> {
    ...
    if (!tok)
      throw new Error('codex non authentifié — lance le device-login (scripts/codex-login.mjs)')
  ...
  async *send(
    ...
    const parseLine = (raw: string): string | null => {
      ...
      try {
        ...
        if (ev.type === 'response.output_text.delta' && typeof ev.delta === 'string')
          return ev.delta
        if (ev.type === 'response.completed') responseId = ev.response?.id
      } catch {
    ...
    return { text, provider: this.id, sessionId: responseId, systemInjected }
  }
}
