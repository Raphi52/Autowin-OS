...
export function MainApp(): React.JSX.Element {
  ...
  function fermerOngletCourant(cible: Tab): void {
    setLayout((courant) => ({
      windows: courant.windows.map((w) =>
        w.id === windowId
          ? {
              ...w,
              tabs: w.tabs.filter((t) => t !== cible),
              active: w.active === cible ? (w.tabs.find((t) => t !== cible) ?? null) : w.active
            }
          : w
      )
    }))
    if (tab === cible) {
      const voisin = mesOnglets.find((t) => t !== cible)
      if (voisin) setTab(voisin)
    }
    void window.api?.appCommand?.('tab_close', { tab: cible })
  }

  function reordonnerOnglet(cible: Tab, index: number): void {
    setLayout((courant) => ouvrirOnglet(courant, cible, windowId, index))
    void window.api?.appCommand?.('tab_move', { tab: cible, window: windowId, index })
  }

  /**
   * L'onglet lache HORS de la fenetre part sur sa propre fenetre, a l'endroit ou on l'a lache —
   * donc sur le 2e ecran si c'est la qu'etait le curseur. La position vient du curseur a l'ECRAN
   * (`screenX/screenY`), seule coordonnee que la page connaisse hors de son cadre.
   */
  function detacherOngletDansSaFenetre(cible: Tab, position: { x: number; y: number }): void {
    void window.api?.appCommand?.('tab_detach', {
      tab: cible,
      bounds: {
        x: Math.round(position.x - 60),
        y: Math.round(position.y - 20),
        width: Math.round(window.outerWidth || 1000),
        height: Math.round(window.outerHeight || 720)
      }
    })
    fermerOngletLocalementApresDetachement(cible)
  }

  function fermerOngletLocalementApresDetachement(cible: Tab): void {
    setLayout((courant) => ({
      windows: courant.windows.map((w) =>
        w.id === windowId
          ? {
    ...
    >
      ...
      <main className={`main${driven ? ' driven' : ''}`} data-driven={driven}>
        <TabBar
          tabs={mesOnglets}
          active={tab}
          onActivate={(cible) => navigate(cible)}
          onClose={fermerOngletCourant}
          onReorder={reordonnerOnglet}
          onDetach={detacherOngletDansSaFenetre}
        />
        {estOuvert('accueil') && (
