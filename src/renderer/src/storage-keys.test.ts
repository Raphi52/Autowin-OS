...
describe('cles LocalStorage Autowin OS', () => {
  ...
  it('imports a distinct legacy profile without replacing current values', () => {
    ...
    expect(storage.getItem(autowinStorageKey('skill-loop.v1'))).toBe('legacy-loop')
    expect(storage.getItem(autowinStorageKey('skill-loop.library.v1'))).toBe('legacy-library')
  })
})
