...
export const NUAGE_FRAGMENT_SHADER = [
  ...
  '  x12.xy -= i1;',
  '  i = mod(i, 289.0);',
  '  vec3 q = permute289(permute289(i.y + vec3(0.0, i1.y, 1.0)) + i.x + vec3(0.0, i1.x, 1.0));',
  ...
  '}',
  // POURQUOI UN BRUIT SIMPLEX, ET PAS UN BRUIT DE VALEUR (2026-08-31, conv-1587).
  //
  // Quatre correctifs successifs (versant linearise, aplat sature, rotation par octave, warp) ont
  // REDUIT les lignes droites sans les faire disparaitre : elles revenaient parce que la cause etait
  // le bruit lui-meme. Un bruit de VALEUR interpole des valeurs aux coins d'une grille CARREE : ses
  // iso-lignes suivent les axes de cette grille, et aucune rotation d'octave ne supprime ces
  // directions privilegiees — elle ne fait que les melanger. `bruitN` est donc desormais un bruit
  // SIMPLEX : grille TRIANGULAIRE et gradients (non des valeurs), donc pas de direction privilegiee
  // a l'origine. Cout comparable (pas de transcendante), plage conservee en [0,1].
  //
  // La rotation par octave RESTE utile contre l'auto-similarite des octaves. Tant que chaque octave
  // subissait la MEME
