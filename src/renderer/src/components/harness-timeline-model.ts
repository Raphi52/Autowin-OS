export type HarnessTimelineEventKind =
  | 'message' | 'injection' | 'decision' | 'tool-call' | 'tool-result' | 'model-response'
  | 'handoff' | 'verdict' | 'gate' | 'retry' | 'cancellation' | 'error' | 'boundary' | 'response-displayed'
export interface HarnessTimelineEvent {
  id: string; kind: HarnessTimelineEventKind; actor: string; label: string; content: string
  detail: string; parentId?: string; provider?: string; model?: string; tokens?: number; costUsd?: number
  timestamp?: string; status?: string; channel?: string; injector?: string; recipient?: string
  inputTokens?: number; outputTokens?: number; cacheReadTokens?: number; durationMs?: number; raw?: unknown
  reasoningEffort?: string; transport?: string; sessionId?: string
  payloads: Array<{ kind: string; content: string; name?: string; mediaType?: string }>
}
export interface HarnessTimelineTurn { id: string; ts: string; events: HarnessTimelineEvent[]; tokens: number; costUsd: number }
export interface HarnessAnomaly {
  kind: 'duplicate-injection' | 'large-injection'; label: string; count: number
  characters: number; impact: number; eventId: string; turnIds: string[]
  fact: string; hypothesis: string; recommendation: string
}
export interface HarnessTimeline { turns: HarnessTimelineTurn[]; anomalies: HarnessAnomaly[]; totalTokens: number; totalCostUsd: number }

export interface HarnessTraceEvent {
  id: string; conversationId: string; turnId: string; parentId?: string; timestamp: string; sequence: number
  type: HarnessTimelineEventKind; status: string; channel: string
  ...
  provider?: { id: string; model?: string; reasoningEffort?: string; transport?: string; sessionId?: string }
  metrics?: { durationMs?: number; inputTokens?: number; outputTokens?: number; cacheReadTokens?: number; costUsd?: number }
}
...
export function buildHarnessTimelineFromTrace(events: HarnessTraceEvent[]): HarnessTimeline {
  ...
  const turns = [...byTurn.entries()].map<HarnessTimelineTurn>(([turnId, turnEvents]) => {
    ...
    const mapped = ordered.map<HarnessTimelineEvent>((event) => {
      ...
      return {
        ...
        detail: event.observation.limitation ? `${provenance} · ${event.observation.limitation}` : provenance,
  ...
  for (const event of events.filter((item) => item.type === 'injection')) {
    ...
    const identity = JSON.stringify({
      payloads: event.payloads.map(({ kind, content: payloadContent, name, mediaType }) => ({ kind, content: payloadContent, name, mediaType })),
      channel: event.channel,
  ...
  for (const { content, occurrences } of injections.values()) {
    ...
    if (occurrences.length > 1) anomalies.push({
      kind: 'duplicate-injection', label: 'Injection répétée', count: occurrences.length,
      characters, impact: characters * (occurrences.length - 1), eventId: occurrences[0].id, turnIds,
      fact: `${occurrences.length} occurrences identiques · ${characters.toLocaleString('fr-FR')} caractères chacune · tours : ${turnIds.join(', ')}`,
      hypothesis: 'Cette répétition peut consommer du contexte si le provider ne la met pas en cache.',
      recommendation: 'Vérifier si cette instruction doit réellement être renvoyée à chaque appel.'
    })
    if (characters >= 12_000) anomalies.push({
      kind: 'large-injection', label: 'Bloc d’instructions volumineux', count: occurrences.length,
      characters, impact: characters, eventId: occurrences[0].id, turnIds,
      fact: `${characters.toLocaleString('fr-FR')} caractères injectés · seuil 12 000`,
      hypothesis: 'Ce volume peut dominer la charge d’entrée, sans prouver que son contenu est inutile.',
      recommendation: 'Vérifier si une version plus courte conserve les contraintes indispensables.'
    })
  }
