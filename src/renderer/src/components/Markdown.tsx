...
import { tokenizeMarkdownCodeBlocks } from './markdown-blocs'
...
function renderMarkdownBlocks(text: string, keyPrefix: string): React.ReactNode[] {
  return tokenizeMarkdownCodeBlocks(text).map((block, index) => {
    ...
    if (block.kind === 'mermaid')
      return (
        // Diagramme Mermaid RENDU dans le fil. Tout le travail delicat (import paresseux,
        // `securityLevel: 'strict'`, etat de chargement, repli sur la source quand la syntaxe est
        // invalide) vit deja dans `ArtifactDiagramPreview` : on le reutilise plutot que de le
        // reecrire, sinon les deux copies divergeraient au premier correctif.
        <div
          key={`${keyPrefix}-mermaid-${index}`}
          className="md-mermaid"
          data-testid="chat-inline-mermaid"
        >
          <ArtifactDiagramPreview source={block.content} />
        </div>
      )
