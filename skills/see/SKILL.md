...
5. **Déposer le compte-rendu** dans la réponse. Sujet durable (invariant visuel, régression) →
   `remember` avec `type: domain`, en citant la clé `SEE-…`.
...
Une clé `SEE-…` n'a de valeur que si elle est retrouvable. Le compte-rendu se dépose dans le fil de
...
comparaison « vs SEE-… » dont la clé n'est ni dans le fil courant ni en mémoire se déclare
...
4. **Meilleure version tracée par sa clé `SEE-…`** (voir « Registre des clés ») : sans elle, la boucle
