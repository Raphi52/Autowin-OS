...
| 5 | **Fichiers de comportement hors dépôt** | `%USERPROFILE%\.claude\settings.json`, `CLAUDE.md` global / projet, `CONSTITUTION.md`, **`AGENTS.md` du dépôt courant** (chargé au même titre que `CLAUDE.md` — `src/main/context-files.ts` ne cherche QUE ces deux noms, il n'existe aucun `AUTOWIN.md`), fiches `memory/` | réflexe global, local-machine, ou **règle que le PROJET dicte à l'agent** (`AGENTS.md` : conventions de build, interdits, périmètre de travail). Inventorie-les avec le scanner de comportements (`src/main/behaviour-files.ts` / vue Comportements) au lieu de deviner un chemin |
