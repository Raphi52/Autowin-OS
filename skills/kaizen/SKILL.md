...
description: >-
  ...
  AND the Autowin CODE — not only the prompts it injects at runtime (`src/main/chat-pilotage-prompt.ts`,
  `src/main/phase-briefs.ts`, `src/main/constitution.ts`, `src/main/intent-phase-routing.ts`) but the orchestration
  loop (`src/main/agent-pilot.ts`), the model adapters (`src/main/providers/*`), the UI (`src/renderer/**`), the
  shared contract (`src/shared/**`) and the telemetry kaizen itself reads —, the TOOLS the agent
...
6. **INTEGRATE — edit the kit to change FUTURE behavior, immediately.** The deliverable IS the edit. Before picking a file, run an explicit **placement analysis** per finding — REASON it, don’t look it up, and surface the "why here" in the STATE table. Three axes: **SCOPE** (below), **ENFORCEMENT** (wired hook+rule > recall fiche — see bullets), **FOLD vs NEW** (extend/retire before adding — see bullets).
   - **SCOPE decides the file AND whether to mirror** — the most-missed axis:
     ...
     - **global sous Claude Code nu** (sessions CLI hors Autowin) → `~/.claude/CLAUDE.md` **+ mirror `CONSTITUTION.md`**. À ne choisir que si le défaut a été observé HORS Autowin.
     - **local** (THIS machine only — RIG hooks, paths, project gates) → the **« Local »** section of `~/.claude/CLAUDE.md`, **NOT mirrored** (a machine-specific fact in the shared `CONSTITUTION.md` pollutes the company kit) — même réserve : hors Autowin uniquement.
     - **project** (un dépôt donné) → le fichier de contexte **gagnant** de ce dépôt, vérifié et non supposé : si `AGENTS.md` existe, éditer son `CLAUDE.md` voisin ne sert à rien. `D:\AutoWinOS` n'a AUCUN des trois au 2026-09-11 — un réflexe « projet » y exige donc de CRÉER `AGENTS.md`, ou de passer au levier 2.
   ...
   - **a triggered reflex / hard rule** → sous Autowin : `src/main/constitution.ts` (levier 2) si le réflexe vaut partout, sinon l'`AGENTS.md` du dépôt visé. Hors Autowin seulement : `CLAUDE.md` (+ `CONSTITUTION.md` **only if global**). **Ne les édite jamais « aussi, au cas où »** : le CLI est lancé sans eux.
   ...
   Then:
   - **Prefer a WIRED trigger** (a hook/gate + a hard rule dans la source RÉELLEMENT injectée — `constitution.ts` sous Autowin) over a passive memory fiche — loading ≠ applying (a fresh fiche was violated twice the same session). Memory fiche = reinforcement, not the primary enforcement.
