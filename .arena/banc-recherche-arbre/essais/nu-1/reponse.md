Voici le fichier `Metier/RechercheArbre.cs` complet. Je ne l'ai ni compilé ni testé, et je n'ai pas mesuré le temps sur 50 000 nœuds.
...
- **Compatibilité :** `Normaliser`, `Correspond`, `Chemins` et `Suivre` n'ont pas changé. La nouvelle forme normalisée, qui retire aussi les accents, sert uniquement à `Rechercher`.
- **Correspondance avec le libellé d'origine :** pour chaque caractère normalisé, je garde sa position dans le libellé d'origine. Cela permet de vérifier la règle 3 (ne pas couper un nombre) et le début de mot sur le vrai libellé.
- **Accents combinants :** un accent qui suit une lettre (ex. `e` + `\u0301`) est rattaché à cette lettre. Il n'est donc pas compté comme séparateur de mot, et on le saute pour trouver le caractère d'avant ou d'après. C'est une interprétation de ma part, la consigne ne le précise pas.
- **Chemins et homonymes :** l'arbre est parcouru dans l'ordre préfixe avec une pile, donc un arbre très profond ne pose pas de problème. Pour chaque nœud, je garde l'indice de son parent. Le chemin est reconstruit à la fin, seulement pour les `max` résultats gardés, à partir des objets réels. Deux homonymes donnent donc deux chemins distincts qui pointent vers les bons nœuds.
- **Faute de frappe (rang 3) :** ce cas n'est cherché que si aucun nœud de l'arbre n'a de rang 0 à 2. On teste les sous-chaînes de longueur |T|−1, |T| et |T|+1, et la comparaison s'arrête dès le deuxième écart, pour tenir le délai sur 50 000 nœuds.
- **Point à vérifier :** en appliquant la règle 4 à la lettre, « 2024/0141 » trouve quand même « Dépôt 2024/01410 » au rang 3 si aucun autre nœud ne correspond. En effet, la sous-chaîne « 202401410 » est à une lettre près et ne coupe aucun nombre. La règle 3 l'exclut bien des rangs 0 à 2, mais pas du rang 3.

Par ailleurs, les connecteurs Gmail, Google Calendar et Google Drive ne sont pas utilisables tant qu'ils ne sont pas autorisés dans les paramètres des connecteurs de claude.ai. Ils ne servaient pas pour cette tâche.